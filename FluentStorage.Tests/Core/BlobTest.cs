...
namespace FluentStorage.Tests.Core {
	public class BlobTest {
		...
		public void Is_root_folder_for_root_folder() {
			Assert.True(new StorageObject("/", StorageObjectType.Folder).IsRootFolder);
		}

		[Fact]
		public void Is_root_folder_for_non_root_folder() {
			Assert.False(new StorageObject("/awesome", StorageObjectType.Folder).IsRootFolder);
		}
		...
		public void Prepend_path(string path, string prefix, string expected) {
			var blob = new StorageObject(path, StorageObjectType.Folder);
